...
import { useGetGalleryQuery, useAddGalleryMutation } from '../redux/services/galleryApi';
...
const Gallery = () => {
  ...
  const { data: response, isLoading } = useGetGalleryQuery({ schoolId }, { skip: !schoolId });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Gallery album added successfully!");
  ...
  return (
    <div className="w-full h-full p-2 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold text-[#0B132B] mb-2">Photo Gallery</h2>
          <p className="text-gray-500 text-sm">Manage school photo albums and event galleries</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {isLoading ? (
          <div className="col-span-full py-12 text-center text-gray-500 font-medium">Loading galleries...</div>
        ) : galleries.length > 0 ? (
          galleries.map((album, index) => (
            <div key={album._id || index} className="bg-white rounded-[20px] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] border border-gray-50 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
              <div className="p-5 border-b border-gray-50">
                <h3 className="text-xl font-bold text-[#0B132B] truncate mb-1">{album.title || "Untitled Album"}</h3>
                <p className="text-gray-500 text-sm line-clamp-2">{album.description || "No description provided."}</p>
              </div>
              <div className="p-5 bg-gray-50/50 flex-grow">
                {album.images && album.images.length > 0 ? (
                    ...
                    {album.images.map((image, imgIndex) => {
                      ...
                      return (
                        ...
                        >
                          <img 
                            src={fileUrl} 
                            alt={`Gallery Preview ${imgIndex + 1}`} 
                  ...
                  <div className="w-full h-48 flex flex-col items-center justify-center bg-gray-100 rounded-xl border border-gray-200 text-gray-400">
                    ...
                    <span className="text-sm font-medium">No Images in Album</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="col-span-full py-16 text-center bg-white rounded-[20px] border border-gray-50 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)]">
            ...
            <p className="text-gray-400 text-sm mt-1">Click on "Create Album" to add some photos.</p>
          </div>
        )}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-[24px] shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
              <h2 className="text-xl font-bold text-[#0B132B]">Create New Album</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Album Title</label>
                <input
                  type="text"
                  name="title"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows="3"
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#0A1629] focus:ring-1 focus:ring-[#0A1629] outline-none transition-all resize-none"
                  required
                />
              </div>
